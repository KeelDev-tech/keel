...
from safe_io import atomic_json, read_json, rows, loads, digest, utc_now, fresh, aware_time, file_lock
    ...
    def read(self, ref):
        ...
        try:
            ...
            for job in jobs:
                ...
                location = job.get('location') if provider == 'ashby' else (job.get('location') or {}).get('name') if provider == 'greenhouse' else (job.get('categories') or {}).get('location')
                normalized[key] = {'identity': list(key), 'url': url, 'title': title,
                                   ...
                                   'advertised_url': job.get('absolute_url') or job.get('hostedUrl') or job.get('jobUrl'),
...
def discover(workspace, *, max_new=200, timeout=120, titles=(), locations=(), reader=None):
    ...
    for source in sources:
        try:
            ...
            for key, posting in postings.items():
                ...
                candidates.append({'role_id': rid, 'company': source.get('company_label') or key[1],
    ...
    # Never ingest a successful subset as a successful run after a hard 429 stop.
    if reader.stopped:
        return {'added': 0, 'status': 'HELD_HTTP_429', 'sources': source_results, 'requests': reader.requests}
    added, duplicates = [], 0
    with queue_lock(timeout=10, owner='public-source:commit'):
...
def verify(workspace, *, limit=100, timeout=120, live=False, reader=None):
    ...
    with queue_lock(timeout=10, owner='public-verify:snapshot'):
        documents, ledger = _documents(workspace)
    ...
    if live:
        ...
        with queue_lock(timeout=10, owner='public-verify:commit'):
            ...
            for path, snapshot, observation in observations:
                rid = snapshot['role_id']
                found = [row for row in rows(current_documents[path]) if row.get('role_id') == rid]
