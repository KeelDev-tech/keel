...
The canonical executable_ready count remains the board's count, including
INTENT holds. The legacy staged-INTENT projection is exposed separately as
eligible_after_intent_resolution, advisory only. It cannot grant execution
or clear any canonical hold. Latest summaries are atomically replaced.
"""
...
def _parse_ts(value):
    try:
        ...
        return ts if ts.tzinfo and ts.utcoffset() is not None else None
    except Exception:
        return None
        ...
        with open(path, "r", encoding="utf-8", errors="replace") as f:
            for line in f:
                ...
                try:
                    ...
                    continue
                if not isinstance(r, dict) or not isinstance(r.get("details", {}), dict):
                    continue
                et = r.get("event_type")
...
def scan_batches_from_rows(rows, anchor):
    ...
    for r in rows:
        if not isinstance(r, dict):
            continue
        if r.get("event_type") != "scan_summary" or r.get("source") != "verify-retry":
            continue
        d = r.get("details") or {}
        if not isinstance(d, dict) or d.get("synthetic_test") or r.get("synthetic_test"):
            continue
        if any(type(d.get(key, 0)) is not int or d.get(key, 0) < 0
               for key in ("scanned", "promoted_ready", "unscored_skipped", "fastpath_live_n")):
            continue
...
def cohort_join(role_ids, rows, anchor, window_h=24):
    ...
    Source-filtered (P1, 2026-09-22, pulse-806 ARM 05; measured by
    pulse-805 ARM 05): the verify-outcome counts (observed_verified /
    observed_dead / observed_gate_blocked) admit ONLY
    source == "verify-retry" events. gate_blocked rows from other
    sources — in the observed regime the materials-owner lane-assignment
    pass emitting details.gate == "materials_missing" — are counted in
    the SEPARATE observed_materials_blocked field with the same
    per-member coverage semantics, and are NEVER mixed into
    verify-outcome math: a lane-assignment abstain is not a verify
    verdict. observed_any / coverage stay any-source telemetry
    observability; coverage_verify / observed_verify_any expose the
    verify-source-only slice so the digest can tell the two apart.
    """
    start = anchor - timedelta(hours=window_h)
    cohort = set(role_ids or [])
    seen = {}            # any-source: member -> set(event_type)
    seen_verify = {}     # verify-retry source only: member -> set(event_type)
    materials = {}       # non-verify gate_blocked: member -> set(gate names)
    for r in rows:
        if (not isinstance(r, dict) or not isinstance(r.get("details", {}), dict)
                or r.get("synthetic_test") or r.get("details", {}).get("synthetic_test")):
            continue
        et = r.get("event_type")
        if et not in PER_ROLE_VERIFY_EVENTS:
            continue
        rid = r.get("role_id")
        ...
        if not ts or ts < start or ts > anchor:
            continue
        is_verify = r.get("source") == "verify-retry"
        seen.setdefault(rid, set()).add(et)
        if is_verify:
            seen_verify.setdefault(rid, set()).add(et)
        elif et == "gate_blocked":
            gate = (r.get("details") or {}).get("gate") or "unknown"
            materials.setdefault(rid, set()).add(gate)
    counts = {et: 0 for et in PER_ROLE_VERIFY_EVENTS}
    for evts in seen_verify.values():
        for et in evts:
            counts[et] += 1
    gate_counts = {}
    for gates in materials.values():
        for g in gates:
            gate_counts[g] = gate_counts.get(g, 0) + 1
    n = len(cohort)
    return {"cohort_size": n, "window_h": window_h,
            "observed_any": len(seen), "coverage": len(seen) / n if n else 0.0,
            "observed_verify_any": len(seen_verify),
            "coverage_verify": len(seen_verify) / n if n else 0.0,
            "observed_verified": counts["lead_verified"],
            "observed_dead": counts["lead_dead"],
            "observed_gate_blocked": counts["gate_blocked"],
            "observed_materials_blocked": len(materials),
            "materials_blocked_gates": gate_counts,
            "note": ("per-role verify telemetry covers every scanned lead "
                     ...
                     "supply, not verdict silence; observed_* outcome "
                     "counts are verify-retry verdicts only; "
                     "observed_materials_blocked counts cohort members "
                     "gate-blocked by non-verify lanes (materials-owner "
                     "materials_missing in the observed regime), reported "
                     "separately and never mixed into verify-outcome math")}
...
def recalibrate_releases(releases, role_ids_by_release, rows, anchor):
    ...
    block = {"version": RECALIBRATION_VERSION, "ok": True,
             ...
             "estimate_basis": "legacy denominator-adjustment heuristic; producer cohort membership not verified",
             "expected_range_is_confidence_interval": False}
...
def _clean_intent_attempt_chain(identity, attempt_apps, attempt_details):
    ...
    if app is None:
        return False  # no journal evidence is not a clean attempt chain
...
def prune(now_ts):
    ...
    for name in sorted(os.listdir(OUT_DIR)):
        if not re.fullmatch(r"(?:snapshot-.*\.json|report-.*\.md|sha256-.*\.txt)", name):
            continue  # never prune unrelated operator files or the latest summary
...
def main():
    ...
    stamp = now.strftime("%Y%m%d-%H%M%S-%f") + "-" + uuid.uuid4().hex[:8]
    ...
    try:
        ...
        print(json.dumps({"ok": False, "stage": "board_summary", "error": "invalid JSON"}))
        return 2
    if not isinstance(summary, dict) or summary.get("execution_authorized") is not False:
        print(json.dumps({"ok": False, "stage": "board_summary", "error": "invalid observer contract"}))
        return 2
    ...
    # The board's executable_ready count is preserved. The projected count
    # is explicitly advisory as eligible_after_intent_resolution.
    try:
        ...
        if gauge.get("ok"):
            # Canonical readiness preserves INTENT holds. The optimistic
            # scheduling projection is diagnostic, never executable supply.
            summary["eligible_after_intent_resolution"] = gauge["executable_ready"]
            summary["intent_held"] = gauge["intent_held"]
            gauge["advisory_only"] = True
            gauge["execution_authorized"] = False
    except Exception as e:  # noqa: BLE001 - observer must not die here
    ...
    fd, temporary = tempfile.mkstemp(prefix=".flow-board-", suffix=".json", dir=OUT_DIR)
    try:
