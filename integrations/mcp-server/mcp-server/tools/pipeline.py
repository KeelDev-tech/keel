...
def keel_run_pipeline(query: str, work_model: str = "", location: str = "",
                      limit: int = 5, verify_urls: bool = False) -> dict:
    ...
    score APPLY-band or better get a prescreen dry-run against a synthetic
    ...
    for role in found["roles"]:
        scored = _scoring.keel_score_role(dict(role))
        step = {
        ...
        if scored.get("recommended_action") == "APPLY":
            brief = (
    ...
    parked = sum(1 for s in stages
                 if isinstance(s.get("prescreen"), dict) and s["prescreen"].get("verdict") == "PARK")
    return {
        "query": query,
        "fixture": True,
        "stages": stages,
