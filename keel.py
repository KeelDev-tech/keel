...
def doctor(workspace):
    ...
    for message in collect(workspace)['warnings']:
        add('Data quality', False, message)
    add('Public boundary', True, 'Preparation only; no paid APIs or application submission are implemented by this CLI.')
    return {'ready_for_local_preparation': all(c['status'] == 'PASS' for c in checks),
...
def main(argv=None):
    ...
    sub.add_parser('doctor', help='offline workspace checks')
