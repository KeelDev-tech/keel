...
def evaluate_readiness(lead, *, now, maximum_age_seconds=900):
    ...
    try:
        ...
        # Expiry is evidence about a *claimed* approval. A lead with no
        # valid approval already carries approval_unverified (above);
        # demanding an expiry timestamp there forces the exporter to invent
        # one, which the honesty doctrine forbids. Evaluate expiry only when
        # an approval is actually claimed -- a claimed approval with a
        # missing/unparseable expiry is still genuinely invalid input and
        # raises below. (ARM 2, 2026-09-19: J-20260919-1858-feed-3021 --
        # second latent input_contract_invalid trigger, masked until the
        # dependencies-shape mismatch was fixed.)
        if lead.get("approval_valid") is True:
            if timestamp(lead.get("approval_expires_at")) <= now:
...
def refill_plan(*, active, permitted_slots, capacity_per_hour, p95_prepare_seconds,
    ...
    # A covered buffer can still have an empty replenishment path. Report it
    # without changing targets, authorizing more checks, or inventing supply.
    state = ("SUPPLY_LIMITED" if infeasible or unmet else "REFILL" if shortfall else
             "BUFFER_COVERED_SUPPLY_EMPTY" if verified_supply == 0 and preparation_wip == 0 else "COVERED")
    return {"state": state,
