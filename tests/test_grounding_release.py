"""Synthetic 0.10 release evidence, source preservation, and additive installation."""
import ast
import hashlib
import json
import os
...
from tools import build_grounding_release as release
from tools import grounding_inventory as sources
from tools import install_grounding as installer
...
def setup_release(tmp_path, monkeypatch):
    ...
    baseline = {'schema': 'keel.grounding.baseline.v1', 'count': 2,
    ...
    files = {'VERSION': b'synthetic-base\n', 'MANIFEST.json': b'historical\n',
             ...
             'docs/GROUNDING_HANDOFF.md': b'SYNTHETIC TEST HANDOFF\n',
             'grounding-base-manifest.json': json.dumps(baseline).encode()}
    ...
    binding = {'schema': 'keel.grounding.test_binding.v1', 'status': 'PASS', 'suite_exit_code': 0,
    ...
    save_json(acceptance, {'schema': 'keel.grounding.acceptance.v1', 'status': 'PASS', 'synthetic': True,
...
def test_transfer_preserves_snapshot_and_adds_evidence_without_false_runtime_claims(tmp_path, monkeypatch):
    ...
    decoded = decode(out/'Keel_0.10.0_Transfer.txt')
    ...
    evidence = json.loads((out/'Keel_0.10.0_Release_Evidence.json').read_text())
    ...
    assert evidence['transfer']['sha256'] == sha((out/'Keel_0.10.0_Transfer.txt').read_bytes())
    assert (out/'Keel_0.10.0_Handoff.txt').read_bytes() == files['docs/GROUNDING_HANDOFF.md']


def test_snapshot_not_second_read_binds_emitted_payload(tmp_path, monkeypatch):
    files, _, _, arguments, out = setup_release(tmp_path, monkeypatch)
    stale = dict(files)
    stale['docs/GROUNDING_HANDOFF.md'] = b'untested content'
...
@pytest.mark.parametrize('entry', ['../outside', 'docs//bad.md', 'tools\\bad.py', 'audit/grounding/stale.json', 'keel_grounding/PATCH_MANIFEST.json', 'source.py'])
...
def setup_patch(tmp_path):
    ...
    patch.mkdir()
    target.mkdir()
    (target/'VERSION').write_bytes(b'unchanged 0.9 prerequisite')
    (target/'MANIFEST.json').write_bytes(b'host historical metadata remains')
    files = {'keel_grounding/added.py': b'synthetic grounding addition',
             'keel_eval/added.py': b'synthetic evaluation addition',
             'docs/GROUNDING_HANDOFF.md': b'synthetic handoff',
             'fixtures/grounding_eval/fixture.json': b'{"synthetic":true}'}
    for name, raw in files.items():
        path = patch/name
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_bytes(raw)
    manifest = {'schema': 'keel.grounding.additive_patch.v1', 'version': '0.10.0',
...
def test_additive_install_readonly_default_and_idempotency(tmp_path):
    ...
    assert not (target/'keel_grounding').exists()
...
def test_install_rejects_conflict_and_tampering_before_any_write(tmp_path, mutation):
    ...
    if mutation == 'base':
        ...
        (patch/'keel_grounding/added.py').write_bytes(b'tampered')
    elif mutation == 'conflict':
        (target/'keel_grounding').mkdir()
        (target/'keel_grounding/added.py').write_bytes(b'host work')
    else:
...
def test_publication_failure_rolls_back_only_this_installs_files(tmp_path, monkeypatch):
    ...
    assert len(calls) == 2
    assert file_bytes(target) == before
    assert not list(target.rglob('.keel-grounding-*'))
...
def test_concurrent_new_destination_is_preserved_without_overwrite(tmp_path, monkeypatch):
    ...
    assert (target/'VERSION').read_bytes() == b'unchanged 0.9 prerequisite'
    assert not list(target.rglob('.keel-grounding-*'))
...
def test_target_parent_symlink_cannot_escape(tmp_path):
    ...
    outside.mkdir()
    (target/'keel_grounding').symlink_to(outside, target_is_directory=True)
    with pytest.raises((ValueError, OSError)):
        installer.install(patch, target, write=True)
...
def test_duplicate_json_manifest_keys_fail_closed(tmp_path):
    patch, target = setup_patch(tmp_path)
    path = patch/installer.MANIFEST
    path.write_text(path.read_text().replace('"version": "0.10.0"', '"version":"0.10.0","version":"0.10.0"'))
...
def test_source_change_after_inspection_rolls_back_partial_addition(tmp_path, monkeypatch):
    ...
    def change_source(*args, **kwargs):
        result = original(*args, **kwargs)
        (patch/'keel_eval/added.py').write_bytes(b'changed after inspection')
        return result
...
def test_rollback_preserves_concurrently_replaced_file(tmp_path, monkeypatch):
    ...
    assert (target/'VERSION').read_bytes() == b'unchanged 0.9 prerequisite'
    assert not (target/'keel_eval/added.py').exists()
