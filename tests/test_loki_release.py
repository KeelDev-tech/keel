"""Synthetic 0.12 release evidence, source preservation, and additive installation."""
import ast
import hashlib
import json
import os
...
from tools import build_loki_release as release
from tools import loki_inventory as sources
from tools import install_loki as installer
...
def setup_release(tmp_path, monkeypatch):
    ...
    baseline = {'schema': 'keel.loki.baseline.v1', 'count': 2,
    ...
    files = {'VERSION': b'synthetic-base\n', 'MANIFEST.json': b'historical\n',
             ...
             'docs/LOKI_HANDOFF.md': b'SYNTHETIC TEST HANDOFF\n',
             'loki-base-manifest.json': json.dumps(baseline).encode()}
    ...
    binding = {'schema': 'keel.loki.test_binding.v1', 'status': 'PASS', 'suite_exit_code': 0,
    ...
    save_json(acceptance, {'schema': 'keel.loki.acceptance.v1', 'status': 'PASS', 'synthetic': True,
...
def test_transfer_preserves_snapshot_and_adds_evidence_without_false_runtime_claims(tmp_path, monkeypatch):
    ...
    decoded = decode(out/'Keel_0.12.0_Transfer.txt')
    ...
    evidence = json.loads((out/'Keel_0.12.0_Release_Evidence.json').read_text())
    ...
    assert evidence['transfer']['sha256'] == sha((out/'Keel_0.12.0_Transfer.txt').read_bytes())
    assert (out/'Keel_0.12.0_Handoff.txt').read_bytes() == files['docs/LOKI_HANDOFF.md']


def test_snapshot_not_second_read_binds_emitted_payload(tmp_path, monkeypatch):
    files, _, _, arguments, out = setup_release(tmp_path, monkeypatch)
    stale = dict(files)
    stale['docs/LOKI_HANDOFF.md'] = b'untested content'
...
@pytest.mark.parametrize('entry', ['../outside', 'docs//bad.md', 'tools\\bad.py', 'audit/loki/stale.json', 'keel_loki/PATCH_MANIFEST.json', 'source.py', 'unknown.py', 'fixtures/other/fixture.json', 'keel_eval/new.py', 'formal/Other.tla'])
...
def test_only_reviewed_formal_paths_are_source_inputs_and_installable(tmp_path, name):
    setup_inventory(tmp_path)
    save_json(tmp_path/sources.ALLOWLIST, [sources.BASELINE, sources.ALLOWLIST, name])
    path = tmp_path/name
    path.parent.mkdir()
    path.write_bytes(b'synthetic bounded model fixture\n')
    assert sources.inventory(tmp_path)['files'][name] == sha(path.read_bytes())
    assert installer._allowed(name) is True
...
def setup_patch(tmp_path):
    ...
    patch.mkdir()
    target.mkdir()
    (target/'VERSION').write_bytes(b'unchanged 0.11 prerequisite')
    (target/'MANIFEST.json').write_bytes(b'host historical metadata remains')
    files = {'keel_loki/added.py': b'synthetic loki addition',
             'tests/added.py': b'synthetic evaluation addition',
             'docs/LOKI_HANDOFF.md': b'synthetic handoff',
             'fixtures/loki/fixture.json': b'{"synthetic":true}'}
    for name, raw in files.items():
        path = patch/name
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_bytes(raw)
    manifest = {'schema': 'keel.loki.additive_patch.v1', 'version': '0.12.0',
...
def test_additive_install_readonly_default_and_idempotency(tmp_path):
    ...
    assert not (target/'keel_loki').exists()
...
def test_install_rejects_conflict_and_tampering_before_any_write(tmp_path, mutation):
    ...
    if mutation == 'base':
        ...
        (patch/'keel_loki/added.py').write_bytes(b'tampered')
    elif mutation == 'conflict':
        (target/'keel_loki').mkdir()
        (target/'keel_loki/added.py').write_bytes(b'host work')
    else:
...
def test_publication_failure_rolls_back_only_this_installs_files(tmp_path, monkeypatch):
    ...
    assert len(calls) == 2
    assert file_bytes(target) == before
    assert not list(target.rglob('.keel-loki-*'))
...
def test_concurrent_new_destination_is_preserved_without_overwrite(tmp_path, monkeypatch):
    ...
    assert (target/'VERSION').read_bytes() == b'unchanged 0.11 prerequisite'
    assert not list(target.rglob('.keel-loki-*'))
...
def test_target_parent_symlink_cannot_escape(tmp_path):
    ...
    outside.mkdir()
    (target/'keel_loki').symlink_to(outside, target_is_directory=True)
    with pytest.raises((ValueError, OSError)):
        installer.install(patch, target, write=True)
...
def test_duplicate_json_manifest_keys_fail_closed(tmp_path):
    patch, target = setup_patch(tmp_path)
    path = patch/installer.MANIFEST
    path.write_text(path.read_text().replace('"version": "0.12.0"', '"version":"0.12.0","version":"0.12.0"'))
