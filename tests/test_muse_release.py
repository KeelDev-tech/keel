"""Synthetic 0.13 release evidence, source preservation, and additive installation."""
import ast
import hashlib
import json
import os
...
from tools import build_muse_release as release
from tools import muse_inventory as sources
from tools import install_muse as installer
...
def setup_release(tmp_path, monkeypatch):
    ...
    baseline = {'schema': 'keel.muse.baseline.v1', 'count': 2,
    ...
    files = {'VERSION': b'synthetic-base\n', 'MANIFEST.json': b'historical\n',
             ...
             'docs/MUSE_HANDOFF.md': b'SYNTHETIC TEST HANDOFF\n',
             'muse-base-manifest.json': json.dumps(baseline).encode()}
    ...
    binding = {'schema': 'keel.muse.test_binding.v1', 'status': 'PASS', 'suite_exit_code': 0,
    ...
    save_json(acceptance, {'schema': 'keel.muse.acceptance.v1', 'status': 'PASS', 'synthetic': True,
...
def test_transfer_preserves_snapshot_and_adds_evidence_without_false_runtime_claims(tmp_path, monkeypatch):
    ...
    decoded = decode(out/'Keel_0.13.0_Transfer.txt')
    ...
    evidence = json.loads((out/'Keel_0.13.0_Release_Evidence.json').read_text())
    ...
    assert evidence['transfer']['sha256'] == sha((out/'Keel_0.13.0_Transfer.txt').read_bytes())
    assert (out/'Keel_0.13.0_Handoff.txt').read_bytes() == files['docs/MUSE_HANDOFF.md']


def test_snapshot_not_second_read_binds_emitted_payload(tmp_path, monkeypatch):
    files, _, _, arguments, out = setup_release(tmp_path, monkeypatch)
    stale = dict(files)
    stale['docs/MUSE_HANDOFF.md'] = b'untested content'
...
@pytest.mark.parametrize('entry', ['../outside', 'docs//bad.md', 'tools\\bad.py', 'audit/muse/stale.json', 'keel_muse/PATCH_MANIFEST.json', 'source.py', 'unknown.py', 'fixtures/other/fixture.json', 'keel_eval/new.py', 'formal/Other.tla'])
...
def test_legacy_formal_paths_cannot_be_added_by_the_muse_patch(tmp_path, name):
    setup_inventory(tmp_path)
    save_json(tmp_path/sources.ALLOWLIST, [sources.BASELINE, sources.ALLOWLIST, name])
    with pytest.raises(ValueError, match='unsupported Muse extension path'):
        sources.inventory(tmp_path)
...
def setup_patch(tmp_path):
    ...
    patch.mkdir()
    target.mkdir()
    (target/'VERSION').write_bytes(b'unchanged 0.12 prerequisite')
    (target/'MANIFEST.json').write_bytes(b'host historical metadata remains')
    files = {'keel_muse/added.py': b'synthetic muse addition',
             'tests/added.py': b'synthetic evaluation addition',
             'docs/MUSE_HANDOFF.md': b'synthetic handoff',
             'fixtures/muse/fixture.json': b'{"synthetic":true}'}
    for name, raw in files.items():
        path = patch/name
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_bytes(raw)
    manifest = {'schema': 'keel.muse.additive_patch.v1', 'version': '0.13.0',
...
def test_additive_install_readonly_default_and_idempotency(tmp_path):
    ...
    assert not (target/'keel_muse').exists()
...
def test_install_rejects_conflict_and_tampering_before_any_write(tmp_path, mutation):
    ...
    if mutation == 'base':
        ...
        (patch/'keel_muse/added.py').write_bytes(b'tampered')
    elif mutation == 'conflict':
        (target/'keel_muse').mkdir()
        (target/'keel_muse/added.py').write_bytes(b'host work')
    else:
...
def test_publication_failure_rolls_back_only_this_installs_files(tmp_path, monkeypatch):
    ...
    assert len(calls) == 2
    assert file_bytes(target) == before
    assert not list(target.rglob('.keel-muse-*'))
...
def test_concurrent_new_destination_is_preserved_without_overwrite(tmp_path, monkeypatch):
    ...
    assert (target/'VERSION').read_bytes() == b'unchanged 0.12 prerequisite'
    assert not list(target.rglob('.keel-muse-*'))
...
def test_target_parent_symlink_cannot_escape(tmp_path):
    ...
    outside.mkdir()
    (target/'keel_muse').symlink_to(outside, target_is_directory=True)
    with pytest.raises((ValueError, OSError)):
        installer.install(patch, target, write=True)
...
def test_duplicate_json_manifest_keys_fail_closed(tmp_path):
    patch, target = setup_patch(tmp_path)
    path = patch/installer.MANIFEST
    path.write_text(path.read_text().replace('"version": "0.13.0"', '"version":"0.13.0","version":"0.13.0"'))
