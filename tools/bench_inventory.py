"""Explicit 0.11 source snapshot, preserving every 0.10 reference member."""
import hashlib
import json
import os
...
BASELINE = "bench-base-manifest.json"
ALLOWLIST = "bench-release-files.json"
...
def baseline_manifest(raw):
    ...
    if (type(value) is not dict or set(value) != {'schema', 'files', 'count'}
            or value['schema'] != 'keel.bench.baseline.v1'):
        raise ValueError("invalid bench baseline")
...
def payload(root=ROOT, *, for_inventory=False):
    ...
    if set(extensions) & set(baseline):
        raise ValueError("bench must not replace baseline files")
    if not {BASELINE, ALLOWLIST}.issubset(extensions):
        raise ValueError("release must bind both manifest files")
    if any(n.startswith('audit/bench/') or n == 'keel_bench/PATCH_MANIFEST.json' for n in extensions):
        raise ValueError("generated evidence cannot be a source input")
    for name in extensions:
        parts = safe_name(name).parts
        allowed = (name in {BASELINE, ALLOWLIST}
                   or len(parts) >= 2 and parts[0] in {'keel_bench', 'tests', 'tools', 'docs'}
                   or len(parts) >= 3 and parts[:2] == ('fixtures', 'bench'))
        if not allowed:
...
def inventory_from_payload(files):
    ...
    return {'schema': 'keel.bench.inventory.v1', 'sha256': hashlib.sha256(raw).hexdigest(),
