#!/usr/bin/env python3
"""Build the source-bound 0.11 TXT reference and additive integration candidate."""
import argparse
import base64
import hashlib
import io
...
from keel_bench import __version__
from tools.bench_inventory import (baseline_manifest, inventory, inventory_from_payload,
                                       payload, read_file, strict_json)
from tools.run_bench_checks import OUTPUTS
...
def check_test_evidence(binding, outputs, observed):
    if (binding.get('schema') != 'keel.bench.test_binding.v1'
...
def check_acceptance(report, observed):
    if (report.get('schema') != 'keel.bench.acceptance.v1'
...
def main(argv=None):
    ...
    preserved = baseline_manifest(files['bench-base-manifest.json'])
    evidence = {'schema': 'keel.bench.release_evidence.v1', 'version': __version__,
    ...
    generated = {'audit/bench/validation.json': encoded_json(evidence),
                 'audit/bench/source-binding.json': binding_raw,
                 'audit/bench/acceptance.json': acceptance_raw,
                 'audit/bench/tests-summary.json': test_outputs['summary.json'],
                 'audit/bench/main-junit.xml': test_outputs['main/junit.xml'],
                 'audit/bench/main-pytest.log': test_outputs['main/pytest.log'],
                 'audit/bench/maintenance-tests.json': test_outputs['maintenance/tests.json'],
                 'audit/bench/maintenance-tests.log': test_outputs['maintenance/tests.log']}
    if set(generated) & set(files) or 'keel_bench/PATCH_MANIFEST.json' in files:
    ...
    manifest = {'schema': 'keel.bench.additive_patch.v1', 'version': __version__,
                'base_files': base, 'files': additions, 'execution_authorized': False,
                'production_deployed': False}
    files['keel_bench/PATCH_MANIFEST.json'] = encoded_json(manifest)
    stream = io.BytesIO()
    stream.write(b'KEEL_TEXT_BUNDLE_V1\n')
    stream.write(json.dumps({'files': len(files), 'scope': '0.11 bench plus unchanged 0.10 reference'}).encode()+b'\n')
...
# KEEL 0.11 BENCHMARK AND HOST VERIFICATION — COMPLETE REFERENCE, ADDITIVE INSTALLER
# Save this WHOLE attachment as Keel_0.11.0_Transfer.txt; no ZIP needed.
# Verify: python3 -B Keel_0.11.0_Transfer.txt --verify-only
# Extract: python3 -B Keel_0.11.0_Transfer.txt --out /existing-parent/new-keel-0.11
# Read docs/BENCH_HANDOFF.md before integration. Never extract over a live tree.
...
'''
    reader = files['tools/transfer_reader.py'].decode().replace(
        ...
        'Read docs/BENCH_HANDOFF.md and docs/BENCH.md')
    ...
    compile(transfer, 'Keel_0.11.0_Transfer.txt', 'exec')
    if inventory() != observed:
        raise ValueError('source changed during packaging')
    out.mkdir(parents=True, exist_ok=False)
    (out/'Keel_0.11.0_Transfer.txt').write_bytes(transfer)
    (out/'Keel_0.11.0_Handoff.txt').write_bytes(files['docs/BENCH_HANDOFF.md'])
    evidence['transfer'] = {'file': 'Keel_0.11.0_Transfer.txt', 'files': len(files), 'bytes': len(transfer),
                            'sha256': sha(transfer), 'source_sha256': sha(raw)}
    (out/'Keel_0.11.0_Release_Evidence.json').write_bytes(encoded_json(evidence))
