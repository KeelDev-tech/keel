#!/usr/bin/env python3
"""Build the source-bound 0.10 TXT reference and additive integration candidate."""
import argparse
import base64
import hashlib
import io
...
from keel_grounding import __version__
from tools.grounding_inventory import (baseline_manifest, inventory, inventory_from_payload,
                                       payload, read_file, strict_json)
from tools.run_grounding_checks import OUTPUTS
...
def check_test_evidence(binding, outputs, observed):
    if (binding.get('schema') != 'keel.grounding.test_binding.v1'
    ...
    suites = list(ET.fromstring(outputs['main/junit.xml']).iter('testsuite'))
...
def check_acceptance(report, observed):
    if (report.get('schema') != 'keel.grounding.acceptance.v1'
...
def main(argv=None):
    ...
    preserved = baseline_manifest(files['grounding-base-manifest.json'])
    evidence = {'schema': 'keel.grounding.release_evidence.v1', 'version': __version__,
    ...
    generated = {'audit/grounding/validation.json': encoded_json(evidence),
                 'audit/grounding/source-binding.json': binding_raw,
                 'audit/grounding/acceptance.json': acceptance_raw,
                 'audit/grounding/tests-summary.json': test_outputs['summary.json'],
                 'audit/grounding/main-junit.xml': test_outputs['main/junit.xml'],
                 'audit/grounding/main-pytest.log': test_outputs['main/pytest.log'],
                 'audit/grounding/maintenance-tests.json': test_outputs['maintenance/tests.json'],
                 'audit/grounding/maintenance-tests.log': test_outputs['maintenance/tests.log']}
    if set(generated) & set(files) or 'keel_grounding/PATCH_MANIFEST.json' in files:
    ...
    manifest = {'schema': 'keel.grounding.additive_patch.v1', 'version': __version__,
                'base_files': base, 'files': additions, 'execution_authorized': False,
                'production_deployed': False}
    files['keel_grounding/PATCH_MANIFEST.json'] = encoded_json(manifest)
    stream = io.BytesIO()
    stream.write(b'KEEL_TEXT_BUNDLE_V1\n')
    stream.write(json.dumps({'files': len(files), 'scope': '0.10 grounding plus unchanged 0.9 reference'}).encode()+b'\n')
...
# KEEL 0.10 EVIDENCE AND PACKET VERIFICATION — COMPLETE REFERENCE, ADDITIVE INSTALLER
# Save this WHOLE attachment as Keel_0.10.0_Transfer.txt; no ZIP needed.
# Verify: python3 -B Keel_0.10.0_Transfer.txt --verify-only
# Extract: python3 -B Keel_0.10.0_Transfer.txt --out /existing-parent/new-keel-0.10
# Read docs/GROUNDING_HANDOFF.md before integration. Never extract over a live tree.
# Only synthetic acceptance ran here. Model inference and live integration did not.
# These checks provide content binding, not independent factual truth or execution authority.
'''
    reader = files['tools/transfer_reader.py'].decode().replace(
        'Read docs/SELF_HOSTED_HANDOFF.md and docs/SELF_HOSTED.md',
        'Read docs/GROUNDING_HANDOFF.md and docs/GROUNDING.md')
    ...
    compile(transfer, 'Keel_0.10.0_Transfer.txt', 'exec')
    if inventory() != observed:
        raise ValueError('source changed during packaging')
    out.mkdir(parents=True, exist_ok=False)
    (out/'Keel_0.10.0_Transfer.txt').write_bytes(transfer)
    (out/'Keel_0.10.0_Handoff.txt').write_bytes(files['docs/GROUNDING_HANDOFF.md'])
    evidence['transfer'] = {'file': 'Keel_0.10.0_Transfer.txt', 'files': len(files), 'bytes': len(transfer),
                            'sha256': sha(transfer), 'source_sha256': sha(raw)}
    (out/'Keel_0.10.0_Release_Evidence.json').write_bytes(encoded_json(evidence))
