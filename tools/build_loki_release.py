#!/usr/bin/env python3
"""Build the source-bound 0.12 TXT reference and additive integration candidate."""
import argparse
import base64
import hashlib
import io
...
from keel_loki import __version__
from tools.loki_inventory import (baseline_manifest, inventory, inventory_from_payload,
                                       payload, read_file, strict_json)
from tools.run_loki_checks import OUTPUTS
...
def check_test_evidence(binding, outputs, observed):
    if (type(binding) is not dict or binding.get('schema') != 'keel.loki.test_binding.v1'
...
def check_acceptance(report, observed):
    if (type(report) is not dict or report.get('schema') != 'keel.loki.acceptance.v1'
            ...
            or report.get('rendered_browser') not in {'NOT_RUN', 'UNAVAILABLE'}
...
def main(argv=None):
    ...
    preserved = baseline_manifest(files['loki-base-manifest.json'])
    evidence = {'schema': 'keel.loki.release_evidence.v1', 'version': __version__,
    ...
    generated = {'audit/loki/validation.json': encoded_json(evidence),
                 'audit/loki/source-binding.json': binding_raw,
                 'audit/loki/acceptance.json': acceptance_raw,
                 'audit/loki/tests-summary.json': test_outputs['summary.json'],
                 'audit/loki/main-junit.xml': test_outputs['main/junit.xml'],
                 'audit/loki/main-pytest.log': test_outputs['main/pytest.log'],
                 'audit/loki/maintenance-tests.json': test_outputs['maintenance/tests.json'],
                 'audit/loki/maintenance-tests.log': test_outputs['maintenance/tests.log']}
    if set(generated) & set(files) or 'keel_loki/PATCH_MANIFEST.json' in files:
    ...
    manifest = {'schema': 'keel.loki.additive_patch.v1', 'version': __version__,
                'base_files': base, 'files': additions, 'execution_authorized': False,
                'production_deployed': False}
    files['keel_loki/PATCH_MANIFEST.json'] = encoded_json(manifest)
    stream = io.BytesIO()
    stream.write(b'KEEL_TEXT_BUNDLE_V1\n')
    stream.write(json.dumps({'files': len(files), 'scope': '0.12 loki plus unchanged 0.11 reference'}).encode()+b'\n')
...
# KEEL 0.12 CAPABILITY LABORATORY AND HOST VERIFICATION — COMPLETE REFERENCE, ADDITIVE INSTALLER
# Save this WHOLE attachment as Keel_0.12.0_Transfer.txt; no ZIP needed.
# Verify: python3 -B Keel_0.12.0_Transfer.txt --verify-only
# Extract: python3 -B Keel_0.12.0_Transfer.txt --out /existing-parent/new-keel-0.12
# Read docs/LOKI_HANDOFF.md before integration. Never extract over a live tree.
...
'''
    reader = files['tools/transfer_reader.py'].decode().replace(
        ...
        'Read docs/LOKI_HANDOFF.md and docs/LOKI.md')
    ...
    compile(transfer, 'Keel_0.12.0_Transfer.txt', 'exec')
    if inventory() != observed:
        raise ValueError('source changed during packaging')
    out.mkdir(parents=True, exist_ok=False)
    (out/'Keel_0.12.0_Transfer.txt').write_bytes(transfer)
    (out/'Keel_0.12.0_Handoff.txt').write_bytes(files['docs/LOKI_HANDOFF.md'])
    evidence['transfer'] = {'file': 'Keel_0.12.0_Transfer.txt', 'files': len(files), 'bytes': len(transfer),
                            'sha256': sha(transfer), 'source_sha256': sha(raw)}
    (out/'Keel_0.12.0_Release_Evidence.json').write_bytes(encoded_json(evidence))
