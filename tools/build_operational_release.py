#!/usr/bin/env python3
"""Build the source-bound 0.14 TXT reference and additive integration candidate."""
import argparse
import base64
import hashlib
import io
...
from keel_operational import __version__
from tools.operational_inventory import (baseline_manifest, inventory, inventory_from_payload,
                                       payload, read_file, strict_json)
from tools.run_operational_checks import OUTPUTS
...
def check_test_evidence(binding, outputs, observed):
    if (type(binding) is not dict or binding.get('schema') != 'keel.operational.test_binding.v1'
            or binding.get('status') != 'PASS' or type(binding.get('suite_exit_code')) is not int
            or binding.get('suite_exit_code') != 0
    ...
    for node in suites:
        ...
        if (len(cases) != int(node.get('tests'))
                or any(child.tag not in {'testcase', 'properties', 'system-out', 'system-err'} for child in node)):
            raise ValueError('JUnit testcase count must match the declared tests')
        if any(child.tag in {'failure', 'error', 'skipped'} for case in cases for child in case):
            raise ValueError('JUnit contains an unsuccessful testcase')
    ...
    if (summary.get('status') != 'PASS' or summary.get('production_deployed') is not False
            ...
            or any(type(row.get('exit_code')) is not int for row in summary.get('runs', []))
...
def check_acceptance(report, observed):
    if (type(report) is not dict or report.get('schema') != 'keel.operational.acceptance.v1'
    ...
    if 'live_canonical_integration' in report and report['live_canonical_integration'] != 'NOT_VERIFIED':
        raise ValueError('live canonical control integration was not verified for this release')
...
def main(argv=None):
    ...
    preserved = baseline_manifest(files['operational-base-manifest.json'])
    evidence = {'schema': 'keel.operational.release_evidence.v1', 'version': __version__,
    ...
    generated = {'audit/operational/validation.json': encoded_json(evidence),
                 'audit/operational/source-binding.json': binding_raw,
                 'audit/operational/acceptance.json': acceptance_raw,
                 'audit/operational/tests-summary.json': test_outputs['summary.json'],
                 'audit/operational/main-junit.xml': test_outputs['main/junit.xml'],
                 'audit/operational/main-pytest.log': test_outputs['main/pytest.log'],
                 'audit/operational/maintenance-tests.json': test_outputs['maintenance/tests.json'],
                 'audit/operational/maintenance-tests.log': test_outputs['maintenance/tests.log']}
    if set(generated) & set(files) or 'keel_operational/PATCH_MANIFEST.json' in files:
    ...
    manifest = {'schema': 'keel.operational.additive_patch.v1', 'version': __version__,
                'base_files': base, 'files': additions, 'execution_authorized': False,
                'production_deployed': False}
    files['keel_operational/PATCH_MANIFEST.json'] = encoded_json(manifest)
    stream = io.BytesIO()
    stream.write(b'KEEL_TEXT_BUNDLE_V1\n')
    stream.write(json.dumps({'files': len(files), 'scope': '0.14 operational plus unchanged 0.13 reference'}).encode()+b'\n')
...
# KEEL 0.14 MUSE OPERATIONS AND RECOVERY — COMPLETE REFERENCE, ADDITIVE INSTALLER
# Save this WHOLE attachment as Keel_0.14.0_Transfer.txt; no ZIP needed.
# Verify: python3 -B Keel_0.14.0_Transfer.txt --verify-only
# Extract: python3 -B Keel_0.14.0_Transfer.txt --out /existing-parent/new-keel-0.14
# Read docs/OPERATIONAL_HANDOFF.md before integration. Never extract over a live tree.
...
'''
    reader = files['tools/transfer_reader.py'].decode().replace(
        ...
        'Read docs/OPERATIONAL_HANDOFF.md and docs/OPERATIONAL.md')
    ...
    compile(transfer, 'Keel_0.14.0_Transfer.txt', 'exec')
    if inventory() != observed:
        raise ValueError('source changed during packaging')
    out.mkdir(parents=True, exist_ok=False)
    (out/'Keel_0.14.0_Transfer.txt').write_bytes(transfer)
    (out/'Keel_0.14.0_Handoff.txt').write_bytes(files['docs/OPERATIONAL_HANDOFF.md'])
    evidence['transfer'] = {'file': 'Keel_0.14.0_Transfer.txt', 'files': len(files), 'bytes': len(transfer),
                            'sha256': sha(transfer), 'source_sha256': sha(raw)}
    (out/'Keel_0.14.0_Release_Evidence.json').write_bytes(encoded_json(evidence))
