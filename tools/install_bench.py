#!/usr/bin/env python3
"""Verify a 0.11 additive patch; write only with --install on an unchanged 0.10 target.
...
from tools.bench_inventory import directory_fd, hash_map, read_file, safe_name, strict_json

MANIFEST = 'keel_bench/PATCH_MANIFEST.json'
VERSION = '0.11.0'
...
def _allowed(name):
    ...
    return (name in {'bench-base-manifest.json', 'bench-release-files.json'}
            or len(parts) >= 2 and parts[0] in {'keel_bench', 'tests', 'tools', 'docs'}
            or len(parts) >= 3 and parts[:2] in {('audit', 'bench'), ('fixtures', 'bench')})
...
def _manifest(root):
    ...
    if (type(value) is not dict or set(value) != {'schema', 'version', 'base_files', 'files',
            ...
            or value['schema'] != 'keel.bench.additive_patch.v1' or value['version'] != VERSION
            or value['execution_authorized'] is not False or value['production_deployed'] is not False):
        raise ValueError('invalid bench patch manifest')
    ...
    if write:
        try:
            for name in pending:
                ...
                temporary = '.keel-bench-'+secrets.token_hex(16)
...
def main(argv=None):
    ...
    try:
        ...
        print('Keel bench additive patch: '+str(exc))
        return 2
