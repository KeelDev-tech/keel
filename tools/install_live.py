#!/usr/bin/env python3
"""Verify an additive patch against Keel0.9 integration reference; install only with --install.
...
MANIFEST = "keel_live/PATCH_MANIFEST.json"
...
def inspect(patch_root, target):
    ...
    for name, expected in sources.items():
        if name in manifest["base_files"] or PurePosixPath(name).parts[0] not in {"keel_sources", "keel_workbench", "keel_live", "tests", "tools", "docs", "audit", "WORKBENCH.md", "workbench-base-manifest.json", "workbench-release-files.json", "live-base-manifest.json", "live-release-files.json"}:
...
def install(patch_root, target, *, write=False):
    root, target = Path(patch_root).absolute(), Path(target).absolute()
    manifest_digest = _hash(_safe(root, MANIFEST, present=True))
    manifest, pending, identical = inspect(root, target)
    if _hash(_safe(root, MANIFEST, present=True)) != manifest_digest:
    ...
    if write:
        try:
            for name in pending:
                ...
                expected = manifest["files"].get(name, manifest_digest)
