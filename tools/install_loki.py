#!/usr/bin/env python3
"""Verify a 0.12 additive patch; write only with --install on an unchanged 0.11 target.
...
from tools.loki_inventory import directory_fd, hash_map, read_file, safe_name, strict_json

MANIFEST = 'keel_loki/PATCH_MANIFEST.json'
VERSION = '0.12.0'
...
def _allowed(name):
    ...
    return (name in {'loki-base-manifest.json', 'loki-release-files.json',
                    'formal/KeelLoki.tla', 'formal/KeelLoki.cfg'}
            or len(parts) >= 2 and parts[0] in {'keel_loki', 'tests', 'tools', 'docs'}
            or len(parts) >= 3 and parts[:2] in {('audit', 'loki'), ('fixtures', 'loki')})
...
def _manifest(root):
    ...
    if (type(value) is not dict or set(value) != {'schema', 'version', 'base_files', 'files',
            ...
            or value['schema'] != 'keel.loki.additive_patch.v1' or value['version'] != VERSION
            or value['execution_authorized'] is not False or value['production_deployed'] is not False):
        raise ValueError('invalid loki patch manifest')
...
def main(argv=None):
    ...
    try:
        ...
        print('Keel loki additive patch: '+str(exc))
        return 2
