#!/usr/bin/env python3
"""Verify a 0.13 additive patch; write only with --install on an unchanged 0.12 target.
...
from tools.muse_inventory import directory_fd, hash_map, read_file, safe_name, strict_json

MANIFEST = 'keel_muse/PATCH_MANIFEST.json'
VERSION = '0.13.0'
...
def _allowed(name):
    ...
    return (name in {'muse-base-manifest.json', 'muse-release-files.json'}
            or len(parts) >= 2 and parts[0] in {'keel_muse', 'tests', 'tools', 'docs'}
            or len(parts) >= 3 and parts[:2] in {('audit', 'muse'), ('fixtures', 'muse')})
...
def _manifest(root):
    ...
    if (type(value) is not dict or set(value) != {'schema', 'version', 'base_files', 'files',
            ...
            or value['schema'] != 'keel.muse.additive_patch.v1' or value['version'] != VERSION
            or value['execution_authorized'] is not False or value['production_deployed'] is not False):
        raise ValueError('invalid muse patch manifest')
    ...
    if write:
        try:
            for name in pending:
                ...
                temporary = '.keel-muse-'+secrets.token_hex(16)
...
def main(argv=None):
    ...
    try:
        ...
        print('Keel muse additive patch: '+str(exc))
        return 2
