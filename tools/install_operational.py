#!/usr/bin/env python3
"""Verify a 0.14 additive patch; write only with --install on an unchanged 0.13 target.
...
from tools.operational_inventory import directory_fd, hash_map, read_file, safe_name, strict_json

MANIFEST = 'keel_operational/PATCH_MANIFEST.json'
VERSION = '0.14.0'
...
def _allowed(name):
    ...
    return (name in {'operational-base-manifest.json', 'operational-release-files.json'}
            or len(parts) >= 2 and parts[0] in {'keel_operational', 'tests', 'tools', 'docs'}
            or len(parts) >= 3 and parts[:2] in {('audit', 'operational'), ('fixtures', 'operational')})
...
def _manifest(root):
    ...
    if (type(value) is not dict or set(value) != {'schema', 'version', 'base_files', 'files',
            ...
            or value['schema'] != 'keel.operational.additive_patch.v1' or value['version'] != VERSION
            or value['execution_authorized'] is not False or value['production_deployed'] is not False):
        raise ValueError('invalid operational patch manifest')
    ...
    if write:
        try:
            for name in pending:
                ...
                temporary = '.keel-operational-'+secrets.token_hex(16)
...
def main(argv=None):
    ...
    try:
        ...
        print('Keel operational additive patch: '+str(exc))
        return 2
