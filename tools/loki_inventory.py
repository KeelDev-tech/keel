"""Explicit 0.12 source snapshot, preserving every 0.11 reference member."""
import hashlib
import json
import os
...
BASELINE = "loki-base-manifest.json"
ALLOWLIST = "loki-release-files.json"
...
def baseline_manifest(raw):
    ...
    if (type(value) is not dict or set(value) != {'schema', 'files', 'count'}
            or value['schema'] != 'keel.loki.baseline.v1'):
        raise ValueError("invalid loki baseline")
...
def payload(root=ROOT, *, for_inventory=False):
    ...
    if set(extensions) & set(baseline):
        raise ValueError("loki must not replace baseline files")
    if not {BASELINE, ALLOWLIST}.issubset(extensions):
        raise ValueError("release must bind both manifest files")
    if any(n.startswith('audit/loki/') or n == 'keel_loki/PATCH_MANIFEST.json' for n in extensions):
        raise ValueError("generated evidence cannot be a source input")
    for name in extensions:
        parts = safe_name(name).parts
        allowed = (name in {BASELINE, ALLOWLIST, 'formal/KeelLoki.tla', 'formal/KeelLoki.cfg'}
                   or len(parts) >= 2 and parts[0] in {'keel_loki', 'tests', 'tools', 'docs'}
                   or len(parts) >= 3 and parts[:2] == ('fixtures', 'loki'))
        if not allowed:
            raise ValueError("unsupported benchmark extension path")
...
def inventory_from_payload(files):
    ...
    return {'schema': 'keel.loki.inventory.v1', 'sha256': hashlib.sha256(raw).hexdigest(),
