"""Explicit 0.13 source snapshot, preserving every 0.12 reference member."""
import hashlib
import json
import os
...
BASELINE = "muse-base-manifest.json"
ALLOWLIST = "muse-release-files.json"
...
def baseline_manifest(raw):
    ...
    if (type(value) is not dict or set(value) != {'schema', 'files', 'count'}
            or value['schema'] != 'keel.muse.baseline.v1'):
        raise ValueError("invalid muse baseline")
...
def payload(root=ROOT, *, for_inventory=False):
    ...
    if set(extensions) & set(baseline):
        raise ValueError("muse must not replace baseline files")
    if not {BASELINE, ALLOWLIST}.issubset(extensions):
        raise ValueError("release must bind both manifest files")
    if any(n.startswith('audit/muse/') or n == 'keel_muse/PATCH_MANIFEST.json' for n in extensions):
        raise ValueError("generated evidence cannot be a source input")
    for name in extensions:
        parts = safe_name(name).parts
        allowed = (name in {BASELINE, ALLOWLIST}
                   or len(parts) >= 2 and parts[0] in {'keel_muse', 'tests', 'tools', 'docs'}
                   or len(parts) >= 3 and parts[:2] == ('fixtures', 'muse'))
        if not allowed:
            raise ValueError("unsupported Muse extension path")
...
def inventory_from_payload(files):
    ...
    return {'schema': 'keel.muse.inventory.v1', 'sha256': hashlib.sha256(raw).hexdigest(),
