"""Explicit 0.14 source snapshot, preserving every 0.13 reference member."""
import hashlib
import json
import os
...
BASELINE = "operational-base-manifest.json"
ALLOWLIST = "operational-release-files.json"
...
def baseline_manifest(raw):
    ...
    if (type(value) is not dict or set(value) != {'schema', 'files', 'count'}
            or value['schema'] != 'keel.operational.baseline.v1'):
        raise ValueError("invalid operational baseline")
...
def payload(root=ROOT, *, for_inventory=False):
    ...
    if set(extensions) & set(baseline):
        raise ValueError("operational must not replace baseline files")
    if not {BASELINE, ALLOWLIST}.issubset(extensions):
        raise ValueError("release must bind both manifest files")
    if any(n.startswith('audit/operational/') or n == 'keel_operational/PATCH_MANIFEST.json' for n in extensions):
        raise ValueError("generated evidence cannot be a source input")
    for name in extensions:
        parts = safe_name(name).parts
        allowed = (name in {BASELINE, ALLOWLIST}
                   or len(parts) >= 2 and parts[0] in {'keel_operational', 'tests', 'tools', 'docs'}
                   or len(parts) >= 3 and parts[:2] == ('fixtures', 'operational'))
        if not allowed:
            raise ValueError("unsupported operational extension path")
...
def inventory_from_payload(files):
    ...
    return {'schema': 'keel.operational.inventory.v1', 'sha256': hashlib.sha256(raw).hexdigest(),
