...
PRIVATE_DIRECTORIES = {
    '.git', '__pycache__', '.pytest_cache', 'data', 'credentials', 'dist',
    'briefs', 'hidden_files', 'audit', 'exports', 'candidates', 'snapshots',
    'remediation', 'launch', 'ledger', 'telemetry',
}
PRIVATE_FILENAMES = {'answer_bank.json', 'applicant_profile.json',
                     'employer_form_patterns.json', 'tray_polish.json'}


def _release_path(name):
    """Reject known runtime/history paths even when accidentally allowlisted."""
    path = PurePosixPath(name)
    # These reviewed Python packages share names with runtime directories.
    source_package = (path.parts[:2] in {('security', 'data'), ('security', 'ledger')}
                      and path.suffix == '.py' and len(path.parts) == 3)
    if (name.casefold() == 'manifest.json'
            or any((part.casefold() in PRIVATE_DIRECTORIES and not (
                       source_package and index == 1 and part in {'data', 'ledger'}))
                   or part.casefold().startswith(('transfer-review-', 'assurance-review-'))
                   or part.casefold().endswith('_history')
                   or part.casefold() == '.env' or part.casefold().startswith('.env.')
                   for index, part in enumerate(path.parts))
            or path.name.casefold() in PRIVATE_FILENAMES
            or path.suffix.casefold() in {'.sqlite', '.sqlite3', '.db', '.pem', '.key'}):
        raise ValueError('reserved, private, historical or runtime path in release allowlist')
